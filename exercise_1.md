...
1. Create a GitHub account if you don't have it already. If you prefer to work on a new fresh Github account, you can create a throway email in Hotmail or Yahoo Mail. 
    ...
    1. Create a new repository called "academy" and <strong>make it Public</strong>.
    ...
    3. Generate an SSH Key and configure it on GitHub to make sure you have the right permissions to Read and Write to the repository.
        ...
        3. Go to GitHub Settings (Click on your image on the top right corner of Github and select "Settings") and then "SSH and GPG keys"
    ...
    4. To add the ssh key locally to use it with git cli, just run the command "ssh-add id_ed25519"
         ...
         NOTE: If you need to manually start the ssh-agent you can run the command ````eval $(ssh-agent -s) ````

## Import the exercise code and create the first workflow

1. Follow the instructions for SSH clone (They are below) to import the backend project that was implemented in the previous modules and create the "main" branch. After this, you should be able to see the base code for our exercise on GitHub.

- **NOTE: Do not run the commands on Github to add the README file, you should execute the commands below to initiate the Github Repository with the code inside the "base_code" folder in the resources folder**
    ```
    # Execute the commands below inside the "base_code" folder
    git init
    git add .
...
5. Now let's list the docker images in the workflow and confirm that indeed the docker image was created.
    ...
    - Transform the previous step instead of creating a new one by using a multi-line command (Example: https://docs.github.com/en/actions/writing-workflows/workflow-syntax-for-github-actions#jobsjob_idstepsrun)x
...
## How to use variables in Github Actions?

You can use GitHub Expressions (e.g. ````${{ ... }}````) to reference different types of context throughout your workflow. These expressions allow you to dynamically access values like environment variables, secrets, inputs, job outputs, and built-in GitHub metadata. 
They can be used in various places such as run commands, with blocks, if conditionals, and step/job names. For example, you can write run: echo ````${{ env.MY_VAR }}```` to use an environment variable, or pass ````${{ secrets.API_KEY }}```` as an input to an action.

You can replace the "env" and "secrets" contexts you saw in the above text, but any other context (https://docs.github.com/en/actions/writing-workflows/choosing-what-your-workflow-does/accessing-contextual-information-about-workflow-runs#about-contexts) you want to access.
...
4. After you have the docker image saved in a file we need to upload it. Let's use the upload-artifact action from Github (Instructions here -> https://github.com/actions/upload-artifact?tab=readme-ov-file#upload-an-individual-file)
   <strong>NOTE: You only need to copy the "upload-artifact" step from the example and adapt the parameters to your needs.</strong>
...
5. Now that the docker image is saved and uploaded, let's bring it back in the second job. Before the step of listing the docker images, you need to create two new steps.
    - First, a simple step with actions/download-artifact (instructions here -> https://github.com/actions/download-artifact?tab=readme-ov-file#download-single-artifact) where you will send the artifact name.
      <strong>NOTE: You only need to copy the "download-artifact" step from the example and adapt the parameters to your needs.</strong>
...
1. Let's create a new step on our workflow to use the action provided by Docker to login on Docker (Use the step in the first example in the Usage section):
    ...
    - <strong>Just adapt the step so that both values are coming from the secrets and not just the password.</strong>
...
2. As you can see, the action just needs two variables, the username and password. We are going to pass these two values to the action from the GitHub Secrets.
    ...
    - Go to the Settings on your GitHub repository and add two repository secrets (Go to "Secrets and variables" -> "Actions" -> "New repository secret").
...
3. Now that we can log in to our Docker account, we can build a new custom image to push the Docker image we built in the previous custom action. Let's create a new custom action to add to our current "docker_build" and "maven_build" actions. You can call it "docker_push" and make sure that:
    - Add two inputs in the custom action, one for the Docker Username and another one for the Docker image reference generated previously.
...
3. Back to the workflow, we need to know to which environment we are deploying to something that we currently have no way to know. Let's add an input to the workflow. The variable should be called "environment" and it should only have the 4 environments that we allow to be chosen. See -> https://docs.github.com/pt/enterprise-cloud@latest/actions/writing-workflows/workflow-syntax-for-github-actions#example-of-onworkflow_dispatchinputs.
   ...
   Note: Please only use the example of the variable "inputs" in the link above that has the "options" key that we want for our environment input (Do not confuse it with the variable "environment" that is also used in the link)
